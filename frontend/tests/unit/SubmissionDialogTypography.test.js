...
describe('submission dialog typography', () => {
  ...
  it('scopes all three dialogs and the teleported Select options to the same scaled token', () => {
    ...
    expect(adminSource).toContain('header="使用者投稿統計"')
  ...
  it('keeps the statistics footer outside the scrollable content with its own action spacing', () => {
    ...
    const closeButton = adminSource.indexOf(
    ...
    expect(styleSource).toMatch(/\.user-submission-stats-dialog .*__content[\s\S]*overflow-y: auto/)
    expect(styleSource).toMatch(
      /\.user-submission-stats-dialog .*__footer[\s\S]*padding: 1rem 1\.5rem/
    )
    expect(styleSource).toMatch(
      /\.user-submission-stats-dialog .*__footer[\s\S]*border-top: 1px solid var\(--border-color\)/
    )
  })
})
